...
```java
Stream<String> stream = Stream.of("福田项目","衢州项目","龙华项目");
```
...
boolean result = list.stream()
            .allMatch(Project::isStudent);
```

### 是否未匹配所有元素：noneMatch Demo5
...
```java
int totalBug = list.stream().reduce(0, (project1,project2)->project1.getBugs()+project2.getBugs());
```
...
```java
int bugs = list.stream().reduce(0, Integer::sum);
```
...
如，将Project中的bugs转换成数值流：

```java
IntStream stream = list.stream().mapToInt(Project::getBugs);
```
...
```java
OptionalInt maxBug = list.stream()
                                .mapToInt(Project::getBugs)
                                .max();
```
